...
| **[Svartálfheim](https://github.com/NorseArchitecture/Svartalfheim)** | The dwarven forge where Mjölnir and Gleipnir were made | `Norse.Primitives.*` — domain value types, identifiers, result parsing, encryption: the unbreakable artifacts every realm carries |
| **[Urðarbrunnr](https://github.com/NorseArchitecture/Urdarbrunnr)** | The Well of Urð at Yggdrasil's roots, where the Norns draw water to sustain the tree and carve fate into its trunk as runes | `Norse.Persistence.*` — the persistence realm: `Norse.Persistence.EntityFramework.*` (entity base types, DbContext foundations, conventions, value converters, migrations chassis) is the live vendor family; other ORMs, native drivers, or document/search stores land as siblings — the record of all that has become |
...
| **[Ratatoskr](https://github.com/NorseArchitecture/Ratatoskr)** | The sly squirrel that races up and down Yggdrasil, carrying slander and secrets between the eagle at the crown and Níðhöggr at the roots — the original message broker | `Norse.Messaging.*` — the messaging realm; `Norse.Messaging.NServiceBus.*` (endpoint configuration, saga infrastructure, message conventions, transport wiring) is the live vendor family: Asgard declares the messaging surface; Ratatoskr carries it |
...
2. **Svartálfheim forges the artifacts** — primitives hardened below the domain, so they compose any domain you define above them.
3. **Urðarbrunnr keeps the record** — the persistence realm: `Norse.Persistence.EntityFramework.*` today (entity base types, DbContext foundations, conventions, value converters, and the migrations chassis), with room for other ORMs, native drivers, or document/search stores as siblings, so persistence starts lawful instead of becoming lawful.
4. **Midgard does the work** — the law, implemented: every infrastructure decision made once, correctly.
5. **Ratatoskr carries the messages** — the messaging realm: `Norse.Messaging.NServiceBus.*` today (endpoint configuration, saga infrastructure, and transport wiring), with room for other brokers or messaging libraries as siblings, so the Æsir declare the messaging surface and every realm picks its own courier.
