...
| **[Nagalfar](https://github.com/NorseArchitecture/Nagalfar)** | The vast ship built from the nails of the dead — launched at Ragnarök to carry the fight, purpose-built to be spent when the battle is won | `Norse.DesignSystem.*` — design tokens, spacing scale, radii, typography, and component primitives: the unglamorous remnants forged seaworthy enough to carry every product UI until the vision is realized |
...
7. **Himinbjorg keeps the roll** — EF persistence for ASP.NET Identity and OpenIddict, sealed server-side: the record of who is known, on which every realm that defers to Heimdall's judgment stands.
...
9. **Bifrost bridges it together** — one orchestration layer composing every resource, from first `dotnet run` on a laptop to deployment.

**Nagalfar stands apart** — no substrate dependencies, no layer to slot into: design tokens, component primitives, and the spacing and typography foundations forged from unglamorous remnants into something seaworthy enough to carry every product UI, purpose-built to be discarded when the vision it scaffolds is realized.
